...
class DatabaseSyncedProfile:
    ...
    def updated_at(self, value):
        if not isinstance(value, datetime.datetime):
            raise ValueError(
                f'Cannot have datatype of {type(value)} for value "updated_at" Expecting datetime.datetime'
            )
    ...
    def avatar_url(self, value):
        ...
        if "http" in value:
            ...
            raise ValueError(
                f'User "avatar_url" expect http:// or https:// or base64 or bytes object, but recieved {value} of type {type(value)} instead!'
            )
