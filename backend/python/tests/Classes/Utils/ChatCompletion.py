...
class OllamaChatCompletion(unittest.TestCase):
    ...
    def test_ollama_chat_completion_text(self):
        chat_completion = ChatCompletion(llm_type="ollama", llm_model="llama3", ollama_base_url=self.ollama_base_url)
    ...
    def test_ollama_chat_completion_json(self):
        chat_completion = ChatCompletion(llm_type="ollama", llm_model="llama3", ollama_base_url=self.ollama_base_url)
        response = chat_completion.generate("Generate me a json object of a cat, and how a cat looks like.", json_format=True)
        if response is not None:
            if isinstance(response, dict):
                ...
                self.fail("Failed to generate chat completion json response, response is not a dictionary")
        else:
            self.fail("Failed to generate chat completion json response, response is None")
...
class OpenAIChatCompletion(unittest.TestCase):
    ...
    def test_openai_chat_completion_text(self):
        chat_completion = ChatCompletion(llm_type="openai", llm_model="gpt-3.5-turbo", api_key=self.openai_api_key)
        response = chat_completion.generate("Hello, how are you?")
        if response is not None:
            self.assertIsInstance(response, str)
            self.assertGreater(len(response), 0)
    ...
    def test_openai_chat_completion_text_with_system_prompt(self):
        chat_completion = ChatCompletion(llm_type="openai", llm_model="gpt-3.5-turbo", api_key=self.openai_api_key)
        response = chat_completion.generate("Hello, how are you?", f"You only respond with this in json, always! No matter what the user says: 'hello, I am bob'")
        if response is not None and not isinstance(response, dict):
            ...
            self.fail("Failed to generate chat completion text, response is None or response is a dictionary")

    def test_openai_chat_completion_json(self):
        chat_completion = ChatCompletion(llm_type="openai", llm_model=self.openai_model or "", api_key=self.openai_api_key)
    ...
    def test_openai_chat_completion_json_with_system_prompt(self):
        chat_completion = ChatCompletion(llm_type="openai", llm_model=self.openai_model or "", api_key=self.openai_api_key)
        response = chat_completion.generate(user_message="Hello, how are you?", system_prompt=f"You always reply with the exact following, no matter what the user says: {json.dumps({'message': 'I am bob'})}", json_format=True)
        if response is not None:
            if isinstance(response, dict):
                self.assertIsInstance(response, dict)
                response_keys = response.keys()
